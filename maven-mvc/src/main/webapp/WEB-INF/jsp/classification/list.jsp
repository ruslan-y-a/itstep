 ...
 <form action="${deleteUrl}" method="post" > 
    <table id="table1" border="1">
	  ...
	  <c:forEach var="parents" items="${list}">     	
	    ...
	    <c:set var="parentshow" value="${parents.parentid != parents.id && parents.parentid>0}"/>	         
	    <c:if test="${not parentshow}">	    
	      <tr>	     	      	    
	    ...
	    <tr class="V${classification.parentid}">	      		
		    <td><input type="checkbox" name="id" value="${classification.id}"></td>		   
		      ...
		      <td >${classification.name}</td>
		       <td >${classification.parentname}</td>	 <!-- ${classification.parentid} ${classification.parentname} -->
		   	   <td>
