...
<%@ taglib tagdir="/WEB-INF/tags" prefix="u" %>
 ...
 <c:if test="${not empty curl}">
    <c:set var="canonical" value="${curl}"/>  <%-- req.getContextPath() --%>
  </c:if>  
    <c:set var="description" value="iShop - online catalog"/>  
  <c:if test="${not empty webpages}">
     <c:set var="description" value="${webpages.description}"/>
     <c:set var="h1" value="${webpages.h1}"/>
  </c:if>
<u:page title="online shoes store. Buy here!" description="${description}" canonical="${canonical}" h1="${h1}" sidemenu2="sidemenu2"> <!-- sidemenu2="sidemenu2" -->
                    ...
                    <c:choose>
					   <c:when test="${not empty curl}">			
						   <c:set var="pageurl" value="${curl}"/>
					   </c:when>
					   <c:otherwise>
						    <c:set var="pageurl" value="/catalog/list.html"/>
					   </c:otherwise>
				   </c:choose>
         
         <c:url var="catalogUrl1" value="${pageurl}">
          ...
          <c:url var="catalogUrl2" value="${pageurl}">
          ...
          <c:url var="catalogUrl3" value="${pageurl}">
       ...
       <c:url var="catalogUrl4" value="${pageurl}">
       ...
       <c:url var="catalogUrl1p" value="${pageurl}">
       ...
       <c:url var="catalogUrl2p" value="${pageurl}">
       ...
       <c:url var="catalogUrl3p" value="${pageurl}">
   ...
   <p></p><p></p>
    <div><strong>Sorting:</strong>  
            ...
            <option></option>
             <c:forEach var="icurrency" items="${currencies}">
                   <c:choose>
                    ...
                    <c:url var="curencyUrl" value="${pageurl}">
                      <c:if test="${not empty param.itemsonpage}"> <c:param name="itemsonpage" value="${param.itemsonpage}"/></c:if>
                      <c:if test="${not empty param.sort}"><c:param name="sort" value="${param.sort}"/></c:if>
     ...
     <div class="end"></div>
           ...
           <c:url var="catalogPageUrlPrev" value="${pageurl}">
             <c:if test="${not empty param.itemsonpage}"><c:param name="itemsonpage" value="${param.itemsonpage}"/></c:if>
             <c:if test="${not empty param.sort}"><c:param name="sort" value="${param.sort}"/></c:if>
           ...
           <div id="navigation fleft">	
	        ...
	        <c:if test="${not empty list}">		
		      ...
		      <c:url var="catalogPageUrlNext" value="${pageurl}">
                 <c:if test="${not empty param.itemsonpage}"><c:param name="itemsonpage" value="${param.itemsonpage}"/></c:if>
                 <c:if test="${not empty param.sort}"><c:param name="sort" value="${param.sort}"/></c:if>
                 <c:if test="${not empty param.page}"><c:param name="page" value="${param.page+1}"/></c:if>
